...
5. **Native platform tests** are release gates on a clean Windows 10/11 x86-64 baseline and macOS 13+ Apple Silicon. They install from the locked dependency set, import direct dependencies, verify the model SHA-256, construct Face Landmarker, open a real webcam, and process synthetic plus live frames for at least 60 seconds. Intel macOS and macOS 11/12 are explicitly not release targets.
...
On each target machine, run three independent 60-second ordinary-preview runs after warm-up, then ten successful Capture Bursts and the complete native smoke suite. Do not hide failures with retries; retain raw aggregate files and a generated Markdown/JSON report. The Windows baseline is an Intel 8th-generation Core i5, 8 GB RAM, CPU-only, 720p webcam. macOS validation uses a supported Apple-Silicon machine and macOS 13 or newer.
