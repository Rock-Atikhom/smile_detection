...
Post-breaker user-authorized fix: commit `ca2a802` gates worker construction on completed verified cache population, adds race/failure/cancellation regressions, and preserves fail-closed integrity behavior. Focused 137/137, full unit 246/246, build, delivery/runtime 24/24, full E2E 52/52, and diff checks pass. Physical device acceptance remains pending.

Post-breaker scoped re-review: clean — spec compliant and task quality approved; no Critical or Important findings. Minor deferred: strengthen the cancellation regression assertion for unchanged `offlineCache` after a late cache result.

Post-breaker Minor closure: commit adds the deferred cancellation assertion so a late cache result cannot flip the cancelled coordinator's `offlineCache` away from `not-ready`. RED/GREEN on the single test (31 tests, 1 targeted pass), focused suites 137/137, full unit 246/246, manifest, format, lint, typecheck, production build, and diff check all pass. Line 348 in `coordinator.test.ts` now asserts `offlineCache: "not-ready"` after a late `finishCache("ready")`.
